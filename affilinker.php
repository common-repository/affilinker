...
Plugin URI: https://www.blasho.com/affilinker/
Description: WordPress plugin to automatically convert keywords into Affiliate Links and to show Affiliate Link Cloud widget - <a href="https://www.blasho.com/affilinker/" target = "_blank">Get all pro-features</a> and take your blog to next level.
Author: BlashO
Version: 2.2
Author URI: https://www.blasho.com/
*/
...
$linkhead = '';
$linkclass = '';
$afflt = array('AffiLinker Pro-Features, take to the next level', 'AffiLinker - General Settings');
...
//					echo '*' . $key . '*[' . str_replace(' ','-',$key) . ']';
                        ...
                        $link = (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/' . $affl_link_term . '/' . str_replace(' ', '-', $key) . '/';
...
function AffiLinker_NavigateToLink() {
    if (1/* !is_admin() */) {
        ...
        $fullURL = (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $reqURL;
        $affl_link_term = get_option("affl_link_term");
		//echo $fullURL;
...
function stripslashes_gpc(&$value)
    ...
    if ($_POST['aal_sent'] == 'ok') {
        ...
        if (0) {
            ...
            $link = rawurldecode($link); // decode url before adding to db
    ...
    if ($_POST['SubmitAll'] == 'Save All Changes') {
        ...
        for ($ids = 0; $ids < $count; $ids++) {
            ...
            $keywordsall[$ids] = preg_replace( "/\s+/", " ", $keywordsall[$ids] );
...
function AffiLinker_CreateMenu() {
    ...
    add_submenu_page(__FILE__, 'Pro-Features', 'Pro-Features', 8, 'affilinkerActivate', 'AFFL_Activate');
...
function AFFL_Activate() {
	global $afflt;

    echo '<div class="wrap"> <div id="icon-options-general" class="icon32"><br /></div>';
    //echo '<h2>' . $afflt[0] . '</h2> <br /> <table class="widefat" cellspacing="7" cellpadding="7" style="background-color:#DBF1FF" width="65%" >';

	echo '<h2><strong>AffiLinker Pro-Features</strong>, take your blog to the next level!</h2> <br />';

    if (get_option("affl_num_enable") == 0) {
    	echo '<table class="widefat" cellspacing="7" cellpadding="7" ><tr style="background-color:white"><td>';
        echo '<h3 style="font-size:16px;">Currently, you are using the free version of AffiLinker.<br/><br/>Activate all the pro-features and take your blog to the next level.</h3>
		<ul style="list-style-type:square">
		  <li style="margin-left:25px;font-size:16px;"><strong>Unlimited Links</strong> - Manage unlimited number of affiliate links</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Take More Control</strong> - Restrict blog posts/pages by number of words</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Earn More Revenue</strong> - Focus on more profitable keywords, hence more revenue</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Attract More Clicks</strong> - Change the hover color, hover background color, font size, underline styles, link title etc.</li>
		</ul>
		<br/>
        <form name="license" method="post" action="http://www.affilinker.com/tester/"> 
        <strong>Enter Your Order Number :</strong> <input type="text" name="affl_user_rcpt_id" size="15" value="" />';
        $ref_url = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        echo '<input type="hidden" name="ref_url" value="'. $ref_url . '" />
        <input type="submit" class="button-primary" value="Activate" name="getlicense" />
        OR <a style="font-size:16px;text-decoration: underline;font-weight:bold;" href="https://www.blasho.com/affilinker-help/#how-to-activate-all-pro-features" target="_blank">More Information &raquo;</a>
		</form>
		<br/>
		<em>Comes with 60-day money back guarantee!</em>
		<br/>
        <p><h3>

		<ul style="list-style-type:square">
		  <li style="margin-left:25px;font-size:16px;">You can <a href="https://www.blasho.com/more-info/affilinker-order-no/" target="_blank" style="text-decoration: underline">get your Order Number here</a> in order to activate all the pro-features.</li>
		  <li style="margin-left:25px;font-size:16px;">Love the plugin? <a href="https://www.blasho.com/affiliate-program/" target="_blank" style="text-decoration: underline">Become an Affiliate</a> (earn 50% commission)</li>
		</ul>
        	</h3></p>
        	<br/>
        <em style="font-size:14px;"><strong>Note:</strong> Currently, with this free version you can manage 6 affiliate links (supports unlimited keywords & unlimited conversions) only.</em>
        </div>';
        echo '</td></tr></table>';
    } else {
    	echo '<table class="widefat" cellspacing="7" cellpadding="7" ><tr style="background-color:white"><td>';
        echo '<h3 style="font-size:16px;">You are using all the pro-features of AffiLinker.</h3>
		<ul style="list-style-type:square">
		  <li style="margin-left:25px;font-size:16px;"><strong>Unlimited Links</strong> - Manage unlimited number of affiliate links</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Take More Control</strong> - Restrict blog posts by number of words</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Prioritize Keywords</strong> - Focus on more profitable keywords, hence more revenue</li>
		  <li style="margin-left:25px;font-size:16px;"><strong>Attract More Clicks</strong> - Change the hover color, hover background color, font size, underline styles, link title etc.</li>
		</ul>
		<br/>
		<p>Good Luck!</p>
        <p><h3>
		<ul style="list-style-type:square">
		  <li style="margin-left:25px;font-size:16px;">Facing any issue with the plugin? <a href="https://www.blasho.com/affilinker-support/" target="_blank" style="text-decoration: underline">Contact Support</a> - <a href="https://www.blasho.com/affilinker-help/" target="_blank" style="text-decoration: underline">Refer to Help</a></li>
		  <li style="margin-left:25px;font-size:16px;">Love the plugin? <a href="https://www.blasho.com/affiliate-program/" target="_blank" style="text-decoration: underline">Become an Affiliate</a> (earn 50% commission)</li>
		</ul>
        	</h3></p> 
        </div>';
...
function AFFL_GeneralSettings() {
	...
	if (get_option("affl_num_enable") == 0) {
		echo '<div class="update-nag"> <a href="./admin.php?page=affilinkerActivate" target="_self" style="text-decoration: underline">Enable pro-features</a> and take your blog to next level!</div>';
	}
	
	echo '<form name="AFFL_gsettings" method="post">';
...
*/
    ...
    echo '<br/><input type="text" name="affl_ignoreposts" value="' . $affl_ignoreposts . '" /> 
				<small>Specify blog post/page IDs seperated by comma, AffiLinker never converts any keyword into link on these blog posts.</small>
				<br/> <br/>';
    ...
    echo '<br/><input type="text" name="affl_onlyposts" value="' . $affl_onlyposts . '"/> 
				<small>Specify blog post/page IDs seperated by comma, AffiLinker converts keyword into link only on these blog posts.</small>  <br/><br/>
				 </td>
		</tr>';
	...
	if (get_option("affl_num_enable") == 0) {
			echo '<td height="50px"><input type="text" name="affl_num_of_wordcount" size="5" value="' . $affl_num_of_wordcount . '" disabled /><br/>';
	} else {
		echo '<td height="50px"><input type="text" name="affl_num_of_wordcount" size="5" value="' . $affl_num_of_wordcount . '" /><br/>';
	}
				echo '<small>Replaces only the blog posts/pages which has more than the specified number of words. <strong>-1</strong> represents no limit.</small>';
				if (get_option("affl_num_enable") == 0) {
					echo '<a href="./admin.php?page=affilinkerActivate" target="_self" style="text-decoration: underline"><small>(Enable Pro-features)</small></a> </td>';
				}
			echo '</td>
		</tr>
		...
		<tr  valign="top">
		    ...
		    if (get_option("affl_num_enable") == 0) {
			    if ($affl_keyword_priority == 1) {
			        ...
			        echo '<input type="radio" name="affl_keyword_priority" value="1" />&nbsp;Enable<br/>
								<input type="radio" name="affl_keyword_priority" value="0" checked="yes" />&nbsp;Disable<br/>';
			    }
			 } else {
			    if ($affl_keyword_priority == 1) {
			        echo '<input type="radio" name="affl_keyword_priority" value="1" checked="yes" />&nbsp;Enable<br/>
								<input type="radio" name="affl_keyword_priority" value="0" />&nbsp;Disable<br/>';
			    } else {
			        echo '<input type="radio" name="affl_keyword_priority" value="1" />&nbsp;Enable<br/>
								<input type="radio" name="affl_keyword_priority" value="0" checked="yes" />&nbsp;Disable<br/>';
			    }
    ...
    echo '<small>When Enabled, AffiLinker turns affiliate links into <em>interactive affiliate links</em> based on the font, size, color, link style settings as specified by you for each keyword. When Disabled, links are displayed in default style matching your blog.</small>
		...
		if (get_option("affl_num_enable") == 0) {
			echo '<td height="50px">http://www.yoursite.com/<input type="text" name="affl_link_term" size="15" value="' . $affl_link_term . '" disabled />/keyword-here/<br/>';
		} else {
			echo '<td height="50px">http://www.yoursite.com/<input type="text" name="affl_link_term" size="15" value="' . $affl_link_term . '" />/keyword-here/<br/>';
		}
				echo '<small>Spice up the links with your own Link Term.</small>';
				if (get_option("affl_num_enable") == 0) {
					echo '<a href="./admin.php?page=affilinkerActivate" target="_self" style="text-decoration: underline"><small>(Enable Pro-features)</small></a> </td>';
				}
			echo '</td>
...
*/
    ...
    /* echo '<table class="widefat" cellspacing="10" cellpadding="10" style="background-color:#DBF1FF" width="100%" >
      <tr  valign="top" >
      <td height="50px"></td>
      <td height="50px">
      <form name="updatech" method="post" action="http://www.affilinker.com/updtch/?vr=' . get_option("affilinker_db_version") . '">
      <input type="submit" style="float:right;" class="button-primary" value="Check For Plugin Updates" name="plugin-updates" />
      </form>';

      if (get_option("affl_updav") == 0)
      {
      echo '<br/><strong style="float:right;">You are already using the latest version of AffiLinker</strong>';
      }
      else if (get_option("affl_updav") == 100)
      {
      echo '<br/><strong style="float:right;">There is a new version of AffiLinker <a href="' . get_option("affl_updurl") . '">here</a></strong>';
...
<!--		<th scope="col" class="manage-column"><center>% of Clicks</center></th> -->
    ...
    foreach ($myrows as $row) {
        ...
        <td><center>
        <?php
/* mysql_query deprecated for php 7+
...
*/
		$query_sum = "SELECT * FROM " . $table_name . " WHERE hit_keyword='$keyword'";
		$all_rows = $wpdb->get_results($query_sum);
		$hit_count_total = 0;
        foreach ($all_rows as $row) {
        	$hit_count_total = $hit_count_total + $row->link_hit_count;
		}
        echo $hit_count_total;
...
function AffiLinker_MainPage() {
    if ((get_option("affl_num_enable") != 0) && ($_REQUEST['affl_user_rcpt_chkd_vld'] != '11')) {
        //echo  '<iframe  style="visibility:hidden;display:none"  height="0" width="0" src="http://www.affilinker.com/tester/?whtuwnt=1&whtuch='. get_option("affl_num_count") . '"></iframe>';
    }
...
*/
    ...
    echo '
		...
		<tr  valign="top" >
			...
			<td height="150px"> <!-- <input type="text" name="keywords" /> --> <textarea wrap="hard" name="keywords" rows=4 cols=65></textarea>
							<br/>
				<small>Example: Canon PowerShot, Canon Camera, canon camera, Digital Camera, Digital camera</small><br/> Keywords are case-sensitive to have more control.</td>
		</tr>
		<tr  valign="top">';
    ...
    echo '</tr>
    	...
    	if (get_option("affl_num_enable") == 0) {
			echo '<h3>Interactive Options (<a href="./admin.php?page=affilinkerActivate" target="_self" style="text-decoration: underline">Enable Pro-features</a>)</h3><br/>';
		} else {
			echo '<h3>Interactive Options</h3><br/>';
		}
		echo '<p>The following are the interactive options which helps you to turn a normal Affiliate Link into Interactive Affiliate Link. You can customize the colors, style, font size of the Link so that it appears unique and catchy to your readers.</p>
		<br/>
		</td>
		</tr>
<!--	<table class="widefat" cellspacing="10" cellpadding="10" style="background-color:#DBF1FF" width="100%"> -->
		<tr  valign="top">
			...
			if (0 /*get_option("affl_num_enable") == 0*/) {
				echo '<input type="text" id="afflink_color" name="link_color" value="" disabled />';
			} else {
				echo '<input class="jscolor {valueElement:link_color}" type="text" id="afflink_color" name="link_color" value="" />';
			}
				echo '<br/>
...
<tr  valign="top">
			...
			if (0 /*get_option("affl_num_enable") == 0*/) {
				echo '<input type="text" name="bg_color" id="affbg_color" value="" disabled />';
			} else {
				echo '<input class="jscolor {valueElement:affbg_color}" type="text" name="bg_color" id="affbg_color" value="" />';
			}
				echo '<br/>
...
<tr  valign="top">
			...
			<td height="60px">';
			if (get_option("affl_num_enable") == 0) {
				echo '<input type="text" name="hover_color" id="affhover_color" value="" disabled />';
			
} else {
				echo '<input class="jscolor {valueElement:affhover_color}" type="text" name="hover_color" id="affhover_color" value="" />';
			}
				echo '<br/>
...
<tr  valign="top">
			...
			<td height="60px">';
			if (get_option("affl_num_enable") == 0) {
				echo '<input type="text" name="hover_bg_color" id="affhover_bg_color" value="" disabled />';
			} else {
				echo '<input class="jscolor {valueElement:affhover_bg_color}" type="text" name="hover_bg_color" id="affhover_bg_color" value="" />';
			}
				echo '<br/>
		...
		<tr  valign="top">
			...
			if (get_option("affl_num_enable") == 0) {
				echo '<select name="font_size" disabled>';
			} else {
				echo '<select name="font_size" >';
			}
				echo '<option value="0" selected>Default Size</option>
		...
		<tr  valign="top">
			...
			<td height="50px">';
			if (0 /*get_option("affl_num_enable") == 0*/) {
				echo '<select name="font_family" disabled>';
			} else {
				echo '<select name="font_family">';
			}
				echo '<option value="0" selected>Default Font</option>
...
<br/>
		...
		<tr  valign="top">
			...
			<td height="50px">';
			if (0) {
				echo '<input type="checkbox" name="link_style_bold" value="1" disabled > <strong>Bold Link</strong><br/>
				<input type="checkbox" name="link_style_italics" value="1" disabled > <em>Italics Link</em><br/>';
			} else {
				echo '<input type="checkbox" name="link_style_bold" value="1"> <strong>Bold Link</strong><br/>
...
<br/><br/>';
	...
	if (get_option("affl_num_enable") == 0) {
		echo '<input type="radio" name="affl_underline_options" value="1" disabled />&nbsp;
		<a href="test" style="text-decoration: none; border-bottom:1px dotted;">Light Dotted</a>

		&nbsp;&nbsp;&nbsp;
		<input type="radio" name="affl_underline_options" value="2" disabled />&nbsp;
		<a href="test" style="text-decoration: none; border-bottom:2px dotted;">Weight Dotted </a>
		<br/><br/>

		<input type="radio" name="affl_underline_options" value="3" disabled />&nbsp;
		...
		echo '<input type="radio" name="affl_underline_options" value="1" />&nbsp;
		<a href="test" style="text-decoration: none; border-bottom:1px dotted;">Light Dotted</a>
...
<tr  valign="top">
			<td height="60px">Title Text :</td>
			<td height="60px">';
			if (get_option("affl_num_enable") == 0) {
				echo '<input type="text" name="hover_title_text" value="" size="65" disabled />';
			} else {
				echo '<input type="text" name="hover_title_text" value="" size="65" />';
			}
				echo '<br/>
				<small>The title text that appears when your visitor hovers the mouse over an affiliate link. Leave it blank, if you don&#39;t want to use.</small>
			</td>
</tr>
...
<!--	</table> -->
	...
	<!--	<table class="widefat" cellspacing="10" cellpadding="10" style="background-color:#DBF1FF" width="100%" > -->
		<tr  valign="top">
			<td height="50px">Enable rel=nofollow : </td>
			<td height="50px">
				<input type="checkbox" name="link_nofollow" value="1" CHECKED> Make it nofollow link
			</td>
		</tr>
		...
		<tr  valign="top">
			...
			<td height="50px">
<input type="checkbox" name="alt_link_keyword" value="1" CHECKED>&nbsp; Hide Link. <br/><small>Example: If the keyword is "buy product" and Link Term is "visit", your affiliate link will be automatically replaced by a professional link: ';
    echo bloginfo('url');
    echo '/visit/buy-product/</small>
			</td>
		</tr>

		<tr  valign="top">
			<td height="80px">Make this a Priority Keyword : </td>
			<td height="80px">';
			if (get_option("affl_num_enable") == 0) {
				echo '<input type="checkbox" name="keyword_priority" value="1" disabled >&nbsp;Priority Keyword<br/>';
			} else {
				echo '<input type="checkbox" name="keyword_priority" value="1" >&nbsp;Priority Keyword<br/>';
			}
				echo '<small>When you make it Priority Keyword, AffiLinker will replace these keywords into links first and then looks for Non-Priority Keywords. This helps when you limit the number of links per post, priority keywords are given more importance than other keywords.';
				if (get_option("affl_num_enable") == 0) {
					echo '<a href="./admin.php?page=affilinkerActivate" target="_self" style="text-decoration: underline">Enable Pro-features</a></small> </td>';
				}
		echo '</tr>
		...
		<tr  valign="top">
			...
			<td height="50px">
				...
				<br/><small>Hint: To enable widget, Go to <em>Appearance->Widgets</em> and Drag-n-drop the AffiLinker Cloud widget to sidebar or footbar</small>
			</td>
		</tr>
...
<table class="widefat" style="box-shadow: 10px 10px 10px 10px #888888;" cellspacing="0" id="active-plugins-table" >
	...
	<?php
            if (0 /*get_option("affl_num_enable") == 0*/) {
	            	echo '<input type="text" size= "7" name="link_color[]" id="affedit_link_color' . $id . '" value="' . $link_color . '" disabled />';
			} else {
	            	echo '<input class="jscolor {valueElement:link_color[]}" type="text" size= "7" name="link_color[]" id="affedit_link_color' . $id . '" value="' . $link_color . '" />';
			}
            echo '<br/>Background Color<br/>';

		if (0 /*get_option("affl_num_enable") == 0*/) {
            echo '<input type="text" size= "7" name="bg_color[]" id="affedit_bg_color' . $id . '" value="' . $bg_color . '" disabled />';
        } else {
            echo '<input class="jscolor {valueElement:bg_color[]}" type="text" size= "7" name="bg_color[]" id="affedit_bg_color' . $id . '" value="' . $bg_color . '" />';
        }
            echo '<br/>Hover Color<br/>';

		if (get_option("affl_num_enable") == 0) {
            echo '<input type="text" size= "7" name="hover_color[]" id="affedit_hover_color' . $id . '" value="' . $hover_color . '" disabled />';
        } else {
            echo '<input class="jscolor {valueElement:hover_color[]}" type="text" size= "7" name="hover_color[]" id="affedit_hover_color' . $id . '" value="' . $hover_color . '" />';
        }

            echo '<br/>Hover Background Color<br/>';

		if (get_option("affl_num_enable") == 0) {
            echo '<input type="text" size= "7" name="hover_bg_color[]" id="affedit_hover_bg_color' . $id . '" value="' . $hover_bg_color . '" disabled />';
        } else {
            echo '<input class="jscolor {valueElement:hover_bg_color[]}" type="text" size= "7" name="hover_bg_color[]" id="affedit_hover_bg_color' . $id . '" value="' . $hover_bg_color . '" />';
        }

        echo '</td>';

	if (get_option("affl_num_enable") == 0) {
        echo '<td> <select name="font_size[]" disabled>';
	} else {
        echo '<td> <select name="font_size[]" >';
	}
	...
	?>
        ...
        foreach ($size_array as $i) {
            if ($i == $font_size) {
            	if ($i == 0)
                	echo '<option value="' . $i . '" selected>Default Size</option>';
                else
            ...
            echo '</select></td>';
	if (0 /*get_option("affl_num_enable") == 0*/) {
        echo '<td> <select name="font_family[]" disabled>';
    } else {
        echo '<td> <select name="font_family[]" >';
    }
		?>
        ...
        if ($keyword_priority == 1) {
            echo '<input type="checkbox" name="keyword_priority[]" value="' . $id . '" CHECKED> Priority Keyword<br/>';
        } else {

        	if (get_option("affl_num_enable") == 0) {
            	echo '<input type="checkbox" name="keyword_priority[]" value="' . $id . '" disabled> Priority Keyword<br/>';
            } else {
				echo '<input type="checkbox" name="keyword_priority[]" value="' . $id . '" > Priority Keyword<br/>';
            }
        }
		...
		if (0 /*get_option("affl_num_enable") == 0*/) {
	        if ($link_style_bold == 1) {
	            echo '<input type="checkbox" name="link_style_bold[]" value="' . $id . '" CHECKED disabled> <strong>Bold</strong><br/>';
	        } else {
	            echo '<input type="checkbox" name="link_style_bold[]" value="' . $id . '" disabled> <strong>Bold</strong><br/>';
	        }
		} else {
	        if ($link_style_bold == 1) {
		...
		if (0 /*get_option("affl_num_enable") == 0*/) {		
	        if ($link_style_italics == 1) {
	            echo '<input type="checkbox" name="link_style_italics[]" value="' . $id . '" CHECKED disabled> <em>Italics</em><br/>';
	        } else {
	            echo '<input type="checkbox" name="link_style_italics[]" value="' . $id . '" disabled> <em>Italics</em><br/>';
	        }
		} else {
	        if ($link_style_italics == 1) {
		...
		if (get_option("affl_num_enable") == 0) {
	        echo '<select name="affl_underline_options[]" disabled>';
	    } else {
   	...
   	if (get_option("affl_num_enable") == 0) {
        ...
        echo '<br/>Title: <input type="text" name="hover_title_text[]" value="' . $hover_title_text . '" size="10" ><br/>';
	}
        ?>
        </td>
        <td>
...
function AffiLinker_InsertAffiliateLinks($content, $affl_comment_callback = 0, $number_of_keywords2replace = 0) {
    ...
    while (1) {
        ...
        if (is_null($myrows)) {
            ...
            foreach ($myrows as $row) {
                //	$link = $row->link;
                $link = (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/' . $affl_link_term . '/' . $row->alt_link_keyword . '/';
                $keywords = $row->keywords;
